...
/** Math utility for the GameKit engine */
public final class GMath {
  ...
  public static final double C4 = (2 * Math.PI) / 3;
  ...
  public static double lerpAngle(double start, double end, double rate) {
    double cos = (1.0 - rate) * Math.cos(start) + rate * Math.cos(end);
    double sin = (1.0 - rate) * Math.sin(start) + rate * Math.sin(end);
